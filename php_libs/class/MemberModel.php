...
class MemberModel extends BaseModel {
    ...
    // 会員情報の検索（同一のメールアドレスが存在するかどうかを確認）
    ...
    // 会員情報の検索（ユーザID）
    public function get_member_data_id($id) { // $id = ID
        $data = [];
        try {
    ...
    // 会員情報の削除
    public function delete_member($id) { // $id = ID
        try {
            $this->pdo->beginTransaction();
    ...
    public function get_member_list($search_key) {
        ...
        try {
            ...
            $i=0;
            $count = $stmh->rowCount(); // 検索件数を取得
            $data = [];                 // 検索結果を多次元配列で受け取る
