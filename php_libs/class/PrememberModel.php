...
class PrememberModel extends BaseModel {
    ...
    public function check_premember($username, $link_pass){
        $data = [];
        try {
            $sql= "SELECT * FROM premember WHERE username = :username AND link_pass = :link_pass limit 1 "; // 引数$username, $link_passとテーブル内のデータ:username, :link_passが一致するものを検索
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':username',  $username,  PDO::PARAM_STR ); // テーブルのパラメータに引数$usernameを割り当てる
            $stmh->bindValue(':link_pass', $link_pass, PDO::PARAM_STR ); // テーブルのパラメータに引数$link_passを割り当てる
            $stmh->execute(); // SQLを実行
            $data = $stmh->fetch(PDO::FETCH_ASSOC); // テーブルから取ってきたデータを$dataに格納する
        } catch (PDOException $Exception) {
            print "エラー：" . $Exception->getMessage();
        }
        return $data;
    ...
    public function delete_premember_and_regist_member($userdata){
        try {
            $this->pdo->beginTransaction();
            /* 仮登録テーブルから1レコードを削除 */
            $sql = "DELETE FROM premember WHERE id = :id"; // テーブルのIDパラメータを使用して仮登録テーブルからデータを削除する
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':id', $userdata['id'], PDO::PARAM_INT ); // テーブルのパラメータに引数$userdata['id']を割り当てる
            $stmh->execute(); // SQLを実行
            /* 会員登録テーブルに1レコードを追加 */
            ...
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':username',   $userdata['username'],   PDO::PARAM_STR ); // テーブルのパラメータに引数$userdata['id']を割り当てる
            $stmh->bindValue(':password',   $userdata['password'],   PDO::PARAM_STR ); // テーブルのパラメータに引数$userdata['password']を割り当てる
            $stmh->bindValue(':last_name',  $userdata['last_name'],  PDO::PARAM_STR ); // テーブルのパラメータに引数$userdata['last_name']を割り当てる
            $stmh->bindValue(':first_name', $userdata['first_name'], PDO::PARAM_STR ); // テーブルのパラメータに引数$userdata['first_name']を割り当てる
            $stmh->bindValue(':birthday',   $userdata['birthday'],   PDO::PARAM_STR ); // テーブルのパラメータに引数$userdata['birthday']を割り当てる
            $stmh->bindValue(':ken',        $userdata['ken'],        PDO::PARAM_INT ); // テーブルのパラメータに引数$userdata['ken']を割り当てる
            $stmh->execute(); // SQLを実行
            $this->pdo->commit();
        } catch (PDOException $Exception) {
            $this->pdo->rollBack();
